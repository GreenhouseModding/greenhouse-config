...
public class GreenhouseConfigTestScreen extends Screen {
	...
	public void tick() {
		...
		if (splitCommonColorWidget.isDirty()) {
			...
			updateColor(config -> config.common().color(), builder::color, color);
		}
		if (splitClientColorWidget.isDirty()) {
			TextColor color = splitClientColorWidget.getColor();
			updateColor(config -> config.client().color(), builder::clientColor, color);
		}
	}
	...
	private void updateColor(Function<SplitConfig, TextColor> colorGetter, Consumer<TextColor> colorSetter, TextColor color) {
